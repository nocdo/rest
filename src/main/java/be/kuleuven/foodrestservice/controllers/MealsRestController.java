...
public class MealsRestController {
    ...
    EntityModel<Meal> postMeal(@RequestBody Meal meal) {
        Meal newMeal = mealsRepository.createMeal(meal);
        return mealToEntityModel(newMeal.getId(), newMeal);
    }

    @PutMapping("/rest/meals/{id}")
    EntityModel<Meal> putMeal(@RequestBody Meal meal, @PathVariable String id) {
        Meal updatedMeal = mealsRepository.updateMeal(meal, id).orElseThrow(() -> new MealNotFoundException(id));
        return mealToEntityModel(id, meal);
    }
